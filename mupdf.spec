...
# {{{ git -C source tag -f 1.27.2-dev 680b230a20d12512198d841b1fc9d7b003afcf9e }}}
...
%global gitdescribepepversion	{{{ git -C source describe --tags --match '[0-9]*' | sed -e 's/-rc\([0-9]*-\)/rc\1dev-/g' -e 's/-rc\([0-9]*\)/rc\1/g' -e 's/^\(.*\)-\([0-9]*\)-g\(.*\)$/\1\2/' -e 's/dev$/dev0/' -e 's/-/./g' }}}
